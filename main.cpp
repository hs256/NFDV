...
void create_trace(NFCompilerVisitor visitor, int index, int np, int resubmit_id, int &branches) {
  trace t(index+1);
  ...
  for (it_path = paths.begin(); it_path != paths.end(); it_path++) {
    ...
    //cout << "No. of constraints " << constraints << endl;
    ...
    if (index < np-1 && last_node->a == "resubmit" ) {
      ...
      create_trace(visitor, index+1, np, resubmit_id_temp, branches);
    } else if (index < np - 1) {
      create_trace(visitor, index+1, np, 0, branches);
    }
    break;
  } else {
    vector<struct tracenode *> path = (*it_path);
  ...
  //cout << "No. of constraints ";
  //for(auto it = cn.begin(); it != cn.end(); it++) {
  //  cout << *it << " " ;
  //}
  //cout << endl;
  cout << "total no. of branches : " << branches << endl;
...
int main(int argc, char *argv[]) {
  ...
  auto start = chrono::high_resolution_clock::now();
  int branches = 0;
  create_trace(visitor, 0, np, 0, branches);
  auto stop = chrono::high_resolution_clock::now();
  auto duration = chrono::duration_cast<chrono::microseconds>(stop - start);
